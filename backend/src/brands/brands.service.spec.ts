...
describe('BrandsService', () => {
  ...
  describe('findAll', () => {
    it('returns all brands', async () => {
      const brands = [{ id: 1, name: 'Nike', slug: 'nike' }, { id: 2, name: 'Adidas', slug: 'adidas' }];
  ...
  describe('create', () => {
    it('creates and returns a brand', async () => {
      ...
      expect(prisma.brand.create).toHaveBeenCalledWith({ data: { name: 'Puma', slug: 'puma' } });
    });

    it('creates a brand with logoUrl', async () => {
      const created = { id: 4, name: 'Reebok', slug: 'reebok', logoUrl: 'https://cdn.test/reebok.png' };
      prisma.brand.create.mockResolvedValue(created);
      const result = await service.create({ name: 'Reebok', slug: 'reebok', logoUrl: 'https://cdn.test/reebok.png' });
  ...
  describe('update', () => {
    it('updates and returns brand', async () => {
      prisma.brand.findUnique.mockResolvedValue({ id: 1, name: 'Nike', slug: 'nike' });
      prisma.brand.update.mockResolvedValue({ id: 1, name: 'Nike Inc', slug: 'nike' });
      const result = await service.update(1, { name: 'Nike Inc' });
      expect(result.name).toBe('Nike Inc');
      expect(prisma.brand.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { name: 'Nike Inc' } });
    });
