...
describe('CartService', () => {
  ...
  describe('addItem', () => {
    ...
    it('throws BadRequestException when stock insufficient (new item)', async () => {
      ...
      prisma.cart.findUnique.mockResolvedValue(cart);
      prisma.cartItem.findUnique.mockResolvedValue(null);
      await expect(service.addItem(1, 5, 5)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('throws BadRequestException when updated quantity exceeds stock', async () => {
      ...
      await expect(service.addItem(1, 5, 2)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('updateItem', () => {
    it('updates quantity of cart item', async () => {
      ...
      const result = (await service.updateItem(1, 5, 4)) as {
        id: number;
        quantity: number;
      };
    ...
    it('throws NotFoundException when cart not found', async () => {
      prisma.cart.findUnique.mockResolvedValue(null);
      await expect(service.updateItem(1, 5, 2)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws NotFoundException when item not in cart', async () => {
      ...
      await expect(service.updateItem(1, 999, 2)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws BadRequestException when requested quantity exceeds stock', async () => {
      ...
      await expect(service.updateItem(1, 5, 10)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('removeItem', () => {
    ...
    it('throws NotFoundException when item not in cart', async () => {
      prisma.cart.findUnique.mockResolvedValue({ id: 1 });
      prisma.cartItem.findFirst.mockResolvedValue(null);
      await expect(service.removeItem(1, 99)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
