...
const reserveGateKey = (whId: number, variantId: number) => `reserve:gate:wh:${whId}:var:${variantId}`;
const RESERVE_GATE_TTL_SECONDS = 60 * 60; // 1 hour — long enough to span a flash sale

export interface CreatedReservation {
  reservationId: number;
  expiresAt: Date;
  items: { variantId: number; quantity: number; lockedPrice: number; lockedMrp: number }[];
  subtotal: number;
}
...
export class CartReservationService {
  ...
  async createReservation(userId: number): Promise<CreatedReservation> {
    ...
    for (const item of cart.items) {
      const activeFlashSaleItem = (item.variant.product.flashSaleItems ?? []).find(
        (fsi) => fsi.flashSale.startsAt <= now2 && fsi.flashSale.endsAt >= now2,
      );
      if (activeFlashSaleItem && (activeFlashSaleItem as any).perUserMaxQty > 0) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of cart.items) {
        ...
        if (isFlash) {
          ...
          const gateResult = await this.redis.tryReserveGate(gKey, item.quantity);
          if (gateResult === false) {
          ...
          if (gateResult === null) {
            ...
            await this.redis.initReserveGate(gKey, sellable, RESERVE_GATE_TTL_SECONDS);
            ...
            if (retry === true) gatesConsumed.push({ key: gKey, qty: item.quantity });
          } else {
        ...
        items.push({ variantId: item.variantId, quantity: item.quantity, lockedPrice, lockedMrp });
   ...
   */
  async validateForCheckout(reservationId: number, userId: number): Promise<{
    valid: boolean;
    reason?: string;
    changedItems?: { variantId: number; lockedPrice: number; currentPrice: number }[];
    reservation?: Prisma.CartReservationGetPayload<{ include: { items: true } }>;
  }> {
      ...
      include: {
        items: {
          include: {
            variant: {
              include: {
                product: { select: { id: true, basePrice: true, discountPct: true } },
              },
            },
          },
        },
    ...
    if (reservation.userId !== userId) return { valid: false, reason: 'Reservation does not belong to user' };
    if (reservation.status !== 'ACTIVE') return { valid: false, reason: `Reservation is ${reservation.status}` };
    if (reservation.expiresAt <= new Date()) return { valid: false, reason: 'Reservation expired' };

    const changedItems: { variantId: number; lockedPrice: number; currentPrice: number }[] = [];
    for (const it of reservation.items) {
      ...
      if (drift > PRICE_DRIFT_TOLERANCE) {
        changedItems.push({ variantId: it.variantId, lockedPrice: it.lockedPrice, currentPrice });
      }
    }
    if (changedItems.length > 0) return { valid: false, reason: 'Price drift', changedItems };
   ...
   */
  async consume(tx: Prisma.TransactionClient, reservationId: number): Promise<void> {
    const { count } = await tx.cartReservation.updateMany({
      where: { id: reservationId, status: 'ACTIVE' },
      data: { status: 'CONSUMED' },
    });
    if (count === 0) {
      throw new ConflictException('Reservation already consumed or expired — duplicate order submission rejected');
    }
  }
  ...
  async releaseById(reservationId: number, userId: number): Promise<void> {
    ...
    if (reservation.userId !== userId) throw new NotFoundException('Reservation not found');
    ...
    if (didExpire && reservation.isFlash) {
      for (const it of reservation.items) {
        await this.redis.releaseReserveGate(reserveGateKey(it.warehouseId, it.variantId), it.quantity);
      }
    }
  }

  /** Return active reservation id for a user's cart, if one exists. */
  async getActiveForUser(userId: number): Promise<{ id: number; expiresAt: Date } | null> {
  ...
  private async enforceConcurrentCap(userId: number): Promise<void> {
    ...
    const toKill = active.slice(0, active.length - MAX_CONCURRENT_RESERVATIONS_PER_USER + 1);
  ...
  async expireOldReservations(): Promise<void> {
    await this.cronLock.runExclusive('cart-reservation-expiry', 50_000, async () => {
      ...
      const flashItems = await this.prisma.$transaction(async (tx) => {
        ...
        `;
        if (rows.length === 0) return [] as { warehouseId: number; variantId: number; quantity: number }[];
        ...
        const flash = flashIds.length > 0
          ? await tx.cartReservationItem.findMany({
              where: { reservationId: { in: flashIds } },
              select: { warehouseId: true, variantId: true, quantity: true },
            })
          : [];
        ...
        this.logger.log(`Expired ${ids.length} cart reservations (batch CTE)`);
        return flash;
      });
